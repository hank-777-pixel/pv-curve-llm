...
def generate_pv_curve(
    ...
    skip_show = os.getenv("PV_CURVE_SKIP_SHOW", "0") == "1"

    if not skip_show:
        print("Displaying curve preview - close the plot window to continue analysis...")
        plt.show()  # CLI: blocking (waits for window close)
        plt.close()
    else:
        # MCP mode: File already saved, skip all GUI operations
        print("Plot saved (MCP mode, skipping interactive display - use file:// URL to view)")
        # Don't call plt.close() - let it be garbage collected to avoid any GUI interaction
